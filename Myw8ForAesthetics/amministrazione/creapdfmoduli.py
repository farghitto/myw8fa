...
def moduloDati(request, id):
    ...
    # # riga 5 - 25y
    can.drawString(25, 635, str(
        dati['cliente']['indirizzo']) + ' ' + str(dati['cliente']['numero_civico']))
    ...
    if dati['cliente_data']['sport_praticato'] != None:
        ...
        can.drawString(239, 390, dati['cliente_data']
                       ['sport_praticato_giorni'])
    ...
    if dati['cliente_data']['numero_sigarette'] != None:
        ...
        can.drawString(248, 366, dati['cliente_data']
                       ['delta_numero_sigarette'])
    ...
    existing_pdf = PdfReader(
        open("static/static_file/modulo_informazioni.pdf", "rb"))
    output = PdfWriter()
...
def moduloAlimenti(request, id):
    ...
    for alimento in alimenti:
        ...
        alimento_presente = next(
            (gusto for gusto in gusti_alimenti if gusto['alimento'] == alimento_da_verificare), None)
    ...
    numero_righe_pagine_interne = numero_righe_tabella - \
        prima_pagina_intevallo - \
        (numero_righe_tabella - ultima_pagina_intervallo)
    dati_paginator = []
    dati_paginator.append(righe_tabella[0:prima_pagina_intevallo])
    dati_paginator.append(righe_tabella[(prima_pagina_intevallo):(
        prima_pagina_intevallo + numero_righe_pagine_interne)])
    dati_paginator.append(
        righe_tabella[(prima_pagina_intevallo + numero_righe_pagine_interne):])
    ...
    for i in pag_alimento.page_range:
        ...
        if not page.has_previous():
            ...
            for idx, riga in enumerate(dati_paginator[i-1]):
                ...
                if riga[1] == ' ':
                    ...
                    lista_stili.append(
                        ('BACKGROUND', (0, idx), (5, idx), colors.red))
                    lista_stili.append(('SPAN', (0, idx), (5, idx)))

            tabella = Table(dati_paginator[i-1],
                            colWidths=(140, 20, 140, 20, 140, 20))
            ...
            # read your existing PDF
            existing_pdf = PdfReader(
                open("static/static_file/modulo_alimenti.pdf", "rb"))
        ...
        # ultima pagina
            ...
            # legeenda tabella
            can.drawString(
                100, 310, 'G = GRADITO       A = ALLERGIA       N = NON GRADITO')
...
def moduloDatiFirmato(request, id):
    ...
    can.drawString(335, 50, str("firmato da " +
                   cliente['cognome'] + " " + cliente['nome']))
    can.drawString(335, 42, str("con email inviata a  " + cliente['email']))
    can.drawString(335, 32, str(
    ...
    nome_file_pdf = f"modulodati-{cliente['cognome']}.pdf"
    percorso_completo_pdf = os.path.join(cartella_destinazione, nome_file_pdf)
    ...
    # finally, write "output" to a real file
    nome_file_pdf_firmato = f"modulodati_firmato-{cliente['cognome']}.pdf"
    ...
    indirizzo_allegati = {
        'file_modulo_dati': percorso_completo_pdf_firmato,
    }

    return indirizzo_allegati


def moduloAlimentiFirmato(request, id):
    ...
    can.drawString(335, 36, str("firmato da " +
                   cliente['cognome'] + " " + cliente['nome']))
    can.drawString(335, 28, str("con email inviata a  " + cliente['email']))
    can.drawString(335, 18, str(
        "in data  " + datetime.now().strftime("%m/%d/%Y, %H:%M:%S")))
    ...
    # read your existing PDF
    cartella_destinazione = 'pdf/' + str(cliente['codice_fiscale'])
    nome_file_pdf = f"moduloalimenti-{cliente['cognome']}.pdf"
    percorso_completo_pdf = os.path.join(cartella_destinazione, nome_file_pdf)

    existing_pdf = PdfReader(open(percorso_completo_pdf, "rb"))
    output = PdfWriter()
    # add the "watermark" (which is the new pdf) on the existing page
    for i in range(0, len(existing_pdf.pages)-1):

        page = existing_pdf.pages[i]
        output.add_page(page)

    page = existing_pdf.pages[len(existing_pdf.pages)-1]
    page.merge_page(new_pdf.pages[0])
    output.add_page(page)

    # finally, write "output" to a real file
    nome_file_pdf_firmato = f"moduloalimenti_firmato-{cliente['cognome']}.pdf"
    percorso_completo_pdf_firmato = os.path.join(
        cartella_destinazione, nome_file_pdf_firmato)
    # finally, write "output" to a real file
    outputStream = open(percorso_completo_pdf_firmato, "wb")
    ...
    indirizzo_allegati = {
        'file_modulo_alimenti': percorso_completo_pdf_firmato,
    }
