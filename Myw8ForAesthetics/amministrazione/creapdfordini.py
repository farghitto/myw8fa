...
def moduloOrdine(request, id):
    ...
    #richiama ordine
    url_backend = settings.BASE_URL + 'ordini/ordini/'+str(id)+'/'
    headers = {"Authorization": f"Token {request.session['auth_token']}"}
    response = requests.get(url_backend, headers=headers)
    if response.status_code == 200:
    ...
    dt = cliente['data_nascita'].strftime("%x")
    ...
    # # riga 4 - 23y
    can.drawString(33,644, cliente['indirizzo'] + ' ' + cliente['numero_civico'])
    ...
    # can.drawString(220,341,str(anag.altezza_cm))
    # can.drawString(405,341,str(anag.peso_desiderato))
    #
    # # dati Biometrici riga 2 - 23y
    # can.drawString(33,318,str(anag.bmi_calcolato))
    # can.drawString(220,318,str(anag.peso_obiettivo))
    # can.drawString(405,318,anag.note)
    
    # Programma
    can.drawString(33,272,programma.nome_programma)
    can.drawString(220,272,str(programma.durata_programma))
    can.drawString(405,272,str(programma.importo))

    # tipologia pagamento riga 1
    can.drawString(170,226,str(programma.importo))
    ...
    if modalita == 'Finanziato':
        ...
        can.drawString(447,203,str(modalita_rateale['importo_rata']))
    #causale
    can.drawString(400,134,str(anag.nome + " " + anag.cognome + " " + programma.nome_programma))
    #nota per indirizzo di spedizione diverso da indirizzo
    


    #data e luogo
    can.drawString(170,53, anag.citta)
    dt = date.today()
    sd = str(dt.day)
    if len(sd) == 1:
        sd = '0' + sd
    ...
    #data e luogo
    can1.drawString(170,63, anag.citta)
    dt = date.today()
    sd = str(dt.day)
    if len(sd) == 1:
        sd = '0' + sd
    ...
    existing_pdf = PdfFileReader(open("static/Cliente.pdf", "rb"))
    ...
    outputStream = open('temporary/' + anag.codice_fiscale + "-" + nomefile, "wb")
